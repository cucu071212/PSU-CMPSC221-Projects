...
public class ScheduleQueries {
    ...
    {
        connection = DBConnection.getConnection();
        ArrayList<ScheduleEntry> waitlistedStudentByClass = new ArrayList<>();
        String status = "waitlisted";
        try
        {
            getWaitlistedStudentsByClass = connection.prepareStatement("select semester, courseCode, studentID, status, timestamp from schedule where semester = ? and courseCode = ? and status = ? order by timestamp");
            getWaitlistedStudentsByClass.setString(1, semester);
            getWaitlistedStudentsByClass.setString(2, courseCode);
            getWaitlistedStudentsByClass.setString(3, status);
            resultSet = getWaitlistedStudentsByClass.executeQuery();
            
            while(resultSet.next())
            {
                ScheduleEntry schedule = new ScheduleEntry(resultSet.getString(1), resultSet.getString(2), resultSet.getString(3), resultSet.getString(4), resultSet.getTimestamp(5));
                waitlistedStudentByClass.add(schedule);
            }
        }
        catch(SQLException sqlException)
        {
    ...
    public static void updateScheduleEntry(ScheduleEntry schedule)
    {
        connection = DBConnection.getConnection();
        
        try
        {
            updateScheduleEntry = connection.prepareStatement("update schedule set status = 'scheduled' where semester = ? and courseCode = ? and studentID = ? and status = 'waitlisted'");
            updateScheduleEntry.setString(1, schedule.getSemester());
